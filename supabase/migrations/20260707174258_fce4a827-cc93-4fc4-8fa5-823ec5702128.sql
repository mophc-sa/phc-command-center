-- ==== 20260706120000_quotations_boq_targets.sql ====
CREATE TYPE public.quotation_status AS ENUM (
  'draft','under_internal_review','approved_for_submission','submitted',
  'follow_up','negotiation','revised','won','lost','expired'
);

CREATE TYPE public.boq_status AS ENUM ('verified','partially_verified','estimated_scope','missing');
...
CREATE TABLE public.boq_items (
  ...
  size TEXT, material TEXT,
  quantity NUMERIC(12,2),
  location TEXT, mounting TEXT, illumination TEXT, finish TEXT,
  unit_rate NUMERIC(14,2), cost_estimate NUMERIC(14,2), selling_price NUMERIC(14,2),
  item_source TEXT,
...
CREATE TABLE public.quotations (
  ...
  issued_date DATE, valid_until DATE,
  last_follow_up_at TIMESTAMPTZ,
  win_loss_reason TEXT, pdf_url TEXT, notes TEXT,
  created_by UUID REFERENCES auth.users(id) ON DELETE SET NULL,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
