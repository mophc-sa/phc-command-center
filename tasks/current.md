...
## بند فعّال — إكمال تشغيل البريد عبر Postmark (2026-09-13، متوقف بطلب المستخدم)

وصلنا إلى: إعادة اختبار زر Send بعد نشر إصلاح #308 (`sales-os-api` v61).
التالي بالترتيب: اختبار الإرسال إلى `@phc-sa.com` والتحقق من dkim/dmarc ← طلب موافقة Postmark ← التأكد من تدوير الرمز الذي ظهر في لقطة شاشة وحذف استعلام curl من SQL Editor ← التقاط الردود (§6 من `docs/implementation/email-sending-setup.md`) ← نشر الواجهة التالي. التفاصيل في أعلى `docs/AI_HANDOFF.md`.
