...
**مدموج على `main`، وغير منشور.** لا migration في هذا العمل إطلاقًا.
...
- 🔴 **الوقاية عند المنبع**: `normalizeContactPayload` تفصل الحقول **قبل** كتابة الصف، ولا تكتب فوق عمود ملأه التعيين، ولا تعيد كتابة `name` إلا عند ثقة عالية. **تعمل في الإنتاج فقط بعد نشر `import-pipeline` — وهذا لم يحدث بعد.**
