      ...
      >
        <header className="flex items-start justify-between gap-3 border-b border-border px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold text-foreground">{t("ask_ai_title" as never)}</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-4 py-3">
          {!asked ? (
            <>
              <p className="mb-2 text-xs text-muted-foreground">{t("ask_ai_intro" as never)}</p>
                ...
                {SUGGESTIONS.map((s) => (
                  <li key={s.en}>
                    <button
                      ...
                      className="w-full rounded-lg border border-border/70 bg-surface/60 px-3 py-2 text-start text-sm text-foreground transition-colors hover:border-border-strong"
                    >
                      {s[lang]}
                    </button>
                  </li>
            ...
            <>
              <div className="mb-2 flex items-baseline justify-between gap-2">
                <span className="rounded-full bg-surface-2 px-2 py-0.5 text-2xs font-medium text-foreground">
            ...
            <div className="py-6 text-center">
              <p className="text-sm text-muted-foreground">{t("ask_ai_not_understood" as never)}</p>
              <Link
                to="/opportunities"
                search={{ q: result.fellThrough ?? "" } as never}
                onClick={onClose}
                className="mt-2 inline-block text-sm text-amber-light hover:underline"
              >
        ...
        >
          <input
            ...
            className="h-9 min-w-0 flex-1 rounded-md border border-border bg-transparent px-3 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus-visible:ring-1 focus-visible:ring-ring"
          />
          <button
            type="submit"
